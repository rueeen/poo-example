...
class PrestamoDAO:
    ...
    def crear(self, id_usuario, id_libro):
        usuario = self.conexion.listar_uno("SELECT * FROM usuarios WHERE id_usuario=%s", (id_usuario,))
        if not usuario:
    ...
    def devolver(self, id_prestamo):
        p = self.conexion.listar_uno("SELECT * FROM prestamos WHERE id_prestamo=%s", (id_prestamo,))
        if not p:
            return False, "Préstamo no existe"
        if p["estado"] == "devuelto":
            return False, "No se puede devolver dos veces"
        self.conexion.ejecutar("UPDATE prestamos SET estado='devuelto', fecha_devolucion=CURDATE() WHERE id_prestamo=%s", (id_prestamo,))
        self.conexion.ejecutar("UPDATE libros SET stock=stock+1 WHERE id_libro=%s", (p["id_libro"],))
    ...
    def cambiar_usuario(self, id_prestamo, nuevo_usuario):
        ...
        u = self.conexion.listar_uno("SELECT * FROM usuarios WHERE id_usuario=%s", (nuevo_usuario,))
        if not u or u["tipo_usuario"] != "suscriptor":
            return False, "Nuevo usuario inválido o no suscriptor"
        self.conexion.ejecutar("UPDATE prestamos SET id_usuario=%s WHERE id_prestamo=%s", (nuevo_usuario, id_prestamo))
    ...
    def cambiar_libro(self, id_prestamo, nuevo_libro):
        ...
        l = self.conexion.listar_uno("SELECT * FROM libros WHERE id_libro=%s", (nuevo_libro,))
        if not l or l["stock"] <= 0:
            return False, "Libro nuevo inválido o sin stock"
        self.conexion.ejecutar("UPDATE libros SET stock=stock+1 WHERE id_libro=%s", (p["id_libro"],))
        self.conexion.ejecutar("UPDATE libros SET stock=stock-1 WHERE id_libro=%s", (nuevo_libro,))
        self.conexion.ejecutar("UPDATE prestamos SET id_libro=%s WHERE id_prestamo=%s", (nuevo_libro, id_prestamo))
