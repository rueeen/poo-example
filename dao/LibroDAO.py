...
class LibroDAO:
    ...
    def crear(self, libro):
        autor = self.conexion.listar_uno(
            "SELECT id_autor FROM autores WHERE id_autor=%s", (libro.id_autor,))
        if not autor:
    ...
    def eliminar(self, id_libro):
        activo = self.conexion.listar_uno(
            "SELECT COUNT(*) total FROM prestamos WHERE id_libro=%s AND estado='activo'", (id_libro,))
        if activo["total"] > 0:
