...
class AutorDAO:
    ...
    def eliminar(self, id_autor):
        rel = self.conexion.listar_uno("SELECT COUNT(*) total FROM libros WHERE id_autor=%s", (id_autor,))
        if rel["total"] > 0:
            return False, "No se puede eliminar: tiene libros asociados"
        self.conexion.ejecutar("DELETE FROM autores WHERE id_autor=%s", (id_autor,))
